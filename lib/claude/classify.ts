    ...
    .array(
      z.object({
        ...
        development_prompts: z
          .array(z.string())
          .describe(
            "0-3 short, concrete questions that would push this idea forward. ONLY when the idea is promising but thin or underdeveloped. Empty array for groceries, list items, complete thoughts, and routine notes."
          ),
      })
    )
    .describe(
...
- Splitting and multi-destination filing are orthogonal: first separate the distinct ideas, then file each idea into its destination(s). Example: "grab almond milk, oh and an idea for the article — why simple tools win" is TWO ideas → one grocery element, one article element.
